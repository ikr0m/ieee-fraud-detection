...
drop_col <- c('V300','V309','V111','V124','V106','V125','V315','V134','V102','V123','V316','V113',
              ...
              'V198', 'V196', 'V155', 'id_02', 'id_30', 'id_31')
...
tem$product_type = paste(tem$ProductCD, tem$TransactionAmt)

tem$uid4 = paste(tem$card1, tem$card2, tem$card3, tem$card5, tem$addr1, tem$addr2, tem$P_emaildomain)
tem$uid5 = paste(tem$card1, tem$card2, tem$card3, tem$card5, tem$addr1, tem$addr2, tem$R_emaildomain)
...
fe_part2 <- tem[, c("acc","card1","card2", "card4", "TransactionAmt", "D15","addr1_addr2","decimal","C13","V258","C1","C14",
                    "addr1__card1", "addr1__card2", "card1__P_emaildomain", "card5__P_emaildomain", "P_emaildomain__addr1", "uid4", "uid5")]
...
fe_part2 <- fe_part2 %>% left_join(plyr::ddply(fe_part2, ~P_emaildomain__addr1, summarise, mean_P_emaildomain__addr1_Trans= mean(TransactionAmt), sd_P_emaildomain__addr1_Trans = sd(TransactionAmt)))
...
fe_part2 <- fe_part2 %>% left_join(plyr::ddply(fe_part2, ~P_emaildomain__addr1, summarise, min_P_emaildomain__addr1_Trans= min(TransactionAmt), max_P_emaildomain__addr1_Trans = max(TransactionAmt)))
...
fe_part2 = fe_part2 %>% left_join(plyr::ddply(fe_part2, ~P_emaildomain__addr1, summarise, mean_P_emaildomain__addr1_dec = mean(decimal,na.rm = T), sd_P_emaildomain__addr1_dec = sd(decimal), P_emaildomain__addr1_dec_q = quantile(decimal,.8,na.rm = T)))
fe_part2 = fe_part2 %>% left_join(plyr::ddply(fe_part2, ~P_emaildomain__addr1, summarise, min_P_emaildomain__addr1_dec = min(decimal,na.rm = T), max_P_emaildomain__addr1_dec = max(decimal)))

fe_part2 <- fe_part2 %>% left_join(plyr::ddply(fe_part2, ~uid4, summarise, mean_uid4_Trans = mean(TransactionAmt), sd_uid4_Trans = sd(TransactionAmt)))
#fe_part2 <- fe_part2 %>% left_join(plyr::ddply(fe_part2, ~uid4, summarise, min_uid4_Trans = min(TransactionAmt), max_uid4_Trans = max(TransactionAmt)))
fe_part2 = fe_part2 %>% left_join(plyr::ddply(fe_part2, ~uid4, summarise, mean_uid4_dec = mean(decimal,na.rm = T), sd_uid4_dec = sd(decimal),uid4_dec_q = quantile(decimal,.8,na.rm = T)))
#fe_part2 = fe_part2 %>% left_join(plyr::ddply(fe_part2, ~uid4, summarise, min_uid4_dec = min(decimal,na.rm = T), max_uid4_dec = max(decimal)))

fe_part2 <- fe_part2 %>% left_join(plyr::ddply(fe_part2, ~uid5, summarise, mean_uid5_Trans = mean(TransactionAmt), sd_uid5_Trans = sd(TransactionAmt)))
#fe_part2 <- fe_part2 %>% left_join(plyr::ddply(fe_part2, ~uid5, summarise, min_uid5_Trans = min(TransactionAmt), max_uid5_Trans = max(TransactionAmt)))
fe_part2 = fe_part2 %>% left_join(plyr::ddply(fe_part2, ~uid5, summarise, mean_uid5_dec = mean(decimal,na.rm = T), sd_uid5_dec = sd(decimal),uid5_dec_q = quantile(decimal,.8,na.rm = T)))
#fe_part2 = fe_part2 %>% left_join(plyr::ddply(fe_part2, ~uid5, summarise, min_uid5_dec = min(decimal,na.rm = T), max_uid5_dec = max(decimal)))


# %% [code]
head(fe_part2)
fe_part2 <- fe_part2[, -c(1:19)]
...
char_features <- tem[,colnames(tem) %in% 
                       c("ProductCD","card1","card2","card3","card4","card5","card6","addr1","addr2","P_emaildomain",
                         ...
                         "id_25","id_26","id_27","id_28","id_29","id_32","id_33","id_34","id_35","id_36",
                         "id_37","id_38", "card1__dist1", "card1__P_emaildomain", "addr1__card1",
                         "card4__dist1", "addr1__card4", "P_emaildomain__C2", "addr1__card2", "P_emaildomain__addr1",
                         "card1__card5", "card5__P_emaildomain", "acc", "uid4", "uid5", "product_type")]
...
char_f <-  c("ProductCD","card1","card2","card3","card4","card5","card6","addr1","addr2","P_emaildomain",
             ...
             "card1__card5", "card5__P_emaildomain", "P_emaildomain__addr1", "product_type", "uid4", "uid5")
...
# Dimension reduction V-variable
...
tem$new9 = tem$decimal_diff * tem$mean_acc_Trans

#tem$decimal_diff_addr1__card1 = tem$decimal - tem$mean_addr1__card1_dec
#tem$new11 = tem$decimal_diff_addr1__card1 * tem$mean_addr1__card1_Trans

#tem$decimal_diff_addr1__card2 = tem$decimal - tem$mean_addr1__card2_dec
#tem$new13 = tem$decimal_diff_addr1__card2 * tem$mean_addr1__card2_Trans

#tem$decimal_diff_card1__P_emaildomain = tem$decimal - tem$mean_card1__P_emaildomain_dec
#tem$new15 = tem$decimal_diff_card1__P_emaildomain * tem$mean_card1__P_emaildomain_Trans

#tem$decimal_diff_card5__P_emaildomain = tem$decimal - tem$mean_card5__P_emaildomain_dec
#tem$new17 = tem$decimal_diff_card5__P_emaildomain * tem$mean_card5__P_emaildomain_Trans
...
write.csv(sub,"submission-937_80.csv",row.names = F)
write.csv(imp,"feature_importance.csv",row.names = F)