...
fe_part2 <- tem[, c("acc","card1","card2","card3", "card4", "TransactionAmt", "id_02", "D15","addr1_addr2","decimal","C13","V258","C1","C14",
                    "addr1__card1", "addr1__card2", "card1__P_emaildomain", "card5__P_emaildomain", "month")]
...
fe_part2 <- fe_part2 %>% left_join(plyr::ddply(fe_part2, ~month, summarise, min_month_Trans= min(TransactionAmt), max_month_Trans = max(TransactionAmt)))
...
char_features <- tem[,colnames(tem) %in% 
                       c("ProductCD","card1","card2","card3","card4","card5","card6","addr1","addr2","P_emaildomain",
                         ...
                         "card1__card5", "card5__P_emaildomain", "acc", "month")]
...
write.csv(sub,"sub_0.9269_70.csv",row.names = F)
