...
train$month = train$date2$mon
test$month = test$date2$mon

train$week = train$date2$week
test$week = test$date2$week

train$wday = wday(train$date2)
test$wday = wday(test$date2)

train$hour = train$date2$hour
test$hour = test$date2$hour

train$wday_hour = factor(paste(train$wday, train$hour, sep = "_"))
test$wday_hour = factor(paste(test$wday, test$hour, sep = "_"))
...
fe_part2 <- tem[, c("acc","card1","card2","card3", "card4", "TransactionAmt", "id_02", "D15","addr1_addr2","decimal","C13","V258","C1","C14", "wday", "wday_hour")]
...
fe_part2 <- fe_part2 %>% left_join(plyr::ddply(fe_part2, ~wday_hour, summarise, mean_wday_hour_Trans = mean(TransactionAmt), sd_wday_hour_Trans = sd(TransactionAmt),
                                               median_wday_hour_Trans= median(TransactionAmt), skewness_wday_hour_Trans = skewness(TransactionAmt), kurtosis_wday_hour_Trans = kurtosis(TransactionAmt)))
...
char_features <- tem[,colnames(tem) %in% 
                       c("ProductCD","card1","card2","card3","card4","card5","card6","addr1","addr2","P_emaildomain",
                         ...
                         "card1__card5", "card5__P_emaildomain", "acc", "DeviceInfo", "wday", "wday_hour")]
...
char_f <-  c("ProductCD","card1","card2","card3","card4","card5","card6","addr1","addr2","P_emaildomain",
             ...
             "card1__card5", "card5__P_emaildomain", "DeviceInfo", "wday", "wday_hour")
...
x <- data.matrix(x[ , 1:141])
tem_sin <- cos(x)
n_pca <- 3
tem_pca <- prcomp_irlba(x, n = n_pca, scale. = TRUE)
tem_pca <- as.data.frame(tem_pca$x)
tem <- data.frame(tem, tem_pca, tem_sin)


rm(tem_pca, x, sample.df.cor, tem_sin); gc(); gc()
...
tr_h2o <- as.h2o(train[ , 1:141])
te_h2o <- as.h2o(test[ , 1:141])
...
write.csv(sub,"sub_0.9358_80_9484.csv",row.names = F)
