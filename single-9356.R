...
train$month = as.factor(train$date2$mon)
test$month = as.factor(test$date2$mon)


train$wday = wday(train$date2)
test$wday = wday(test$date2)
...
train$Csum <- rowSums(train[, 17:30])
test$Csum <- rowSums(test[, 17:30])

train$n_na_D <- rowSums(is.na(train[, 31:45]))
test$n_na_D <- rowSums(is.na(test[, 31:45]))

train$Dsum <- rowSums(train[, 31:45])
test$Dsum <- rowSums(test[, 31:45])
...
tem <- train %>% bind_rows(test) %>%
  mutate(hr = floor( (TransactionDT / 3600) %% 24 ),
         wday_houday = floor( (TransactionDT / 3600 / 24) %% 7)
  ) %>%
...
tem <- tem %>%
  mutate(
    ...
    card1__dist2 = unlist(map2(.x = card1, .y = dist2, .f = function(x,y){
      y <- str_c(x, y, sep = "_")
      return(y)
    })),
    ...
    addr2__card4 = unlist(map2(.x = addr2, .y = card4, .f = function(x,y){
      y <- str_c(x, y, sep = "_")
      return(y)
    })),
    ...
    addr2__card2 = unlist(map2(.x = addr2, .y = card2, .f = function(x,y){
      y <- str_c(x, y, sep = "_")
      return(y)
    })),
...
fe_part2 <- tem[, c("acc","card1","card2","card3", "card4", "TransactionAmt", "id_02", "D15","addr1_addr2","decimal","C13","V258","C1","C14", "month", "wday", "PRmailTF", "D0")]
...
fe_part2 <- fe_part2 %>% left_join(plyr::ddply(fe_part2, ~wday, summarise, mean_wday_Trans = mean(TransactionAmt), sd_wday_Trans = sd(TransactionAmt),
                                               median_wday_Trans= median(TransactionAmt), skewness_wday_Trans = skewness(TransactionAmt), kurtosis_wday_Trans = kurtosis(TransactionAmt)))

fe_part2 <- fe_part2 %>% left_join(plyr::ddply(fe_part2, ~month, summarise, mean_month_Trans = mean(TransactionAmt), sd_month_Trans = sd(TransactionAmt),
                                               median_month_Trans= median(TransactionAmt), skewness_month_Trans = skewness(TransactionAmt), kurtosis_month_Trans = kurtosis(TransactionAmt)))

fe_part2 <- fe_part2 %>% left_join(plyr::ddply(fe_part2, ~D0, summarise, mean_D0_Trans = mean(TransactionAmt), sd_D0_Trans = sd(TransactionAmt),
                                               median_D0_Trans= median(TransactionAmt), skewness_D0_Trans = skewness(TransactionAmt), kurtosis_D0_Trans = kurtosis(TransactionAmt)))
...
char_features <- tem[,colnames(tem) %in% 
                       c("ProductCD","card1","card2","card3","card4","card5","card6","addr1","addr2","P_emaildomain",
                         ...
                         "card1__card5", "card5__P_emaildomain", "acc", "DeviceInfo", "month", "wday", "PRmailTF", "D0")]
...
char_f <-  c("ProductCD","card1","card2","card3","card4","card5","card6","addr1","addr2","P_emaildomain",
             ...
             "card1__card5", "card5__P_emaildomain", "DeviceInfo", "month", "wday", "PRmailTF", "D0")
...
tem <- tem %>% group_by(month, card1) %>% mutate(mean_TransactionAmt_month_card1 = mean(TransactionAmt, na.rm = TRUE),
                                                 median_TransactionAmt_month_card1 = median(TransactionAmt, na.rm = TRUE),
                                                 skewness_TransactionAmt_month_card1 = skewness(TransactionAmt, na.rm = TRUE),
                                                 kurtosis_TransactionAmt_month_card1 = kurtosis(TransactionAmt, na.rm = TRUE),
                                                 sd_TransactionAmt_month_card1 = sd(TransactionAmt, na.rm = TRUE)) %>%
  ungroup() %>%
  mutate(TransactionAmt_to_mean_month_card1 = TransactionAmt/mean_TransactionAmt_month_card1,
         TransactionAmt_to_sd_month_card1 = TransactionAmt/sd_TransactionAmt_month_card1,
         TransactionAmt_to_skewness_month_card1 = TransactionAmt/skewness_TransactionAmt_month_card1,
         TransactionAmt_to_kurtosis_month_card1 = TransactionAmt/kurtosis_TransactionAmt_month_card1,
         TransactionAmt_subs_month_card1 = TransactionAmt - mean_TransactionAmt_month_card1,
         TransactionAmt_subs_month_card1_median = TransactionAmt - median_TransactionAmt_month_card1)


tem <- tem %>% group_by(wday, card1) %>% mutate(mean_TransactionAmt_wday_card1 = mean(TransactionAmt, na.rm = TRUE),
                                                median_TransactionAmt_wday_card1 = median(TransactionAmt, na.rm = TRUE),
                                                skewness_TransactionAmt_wday_card1 = skewness(TransactionAmt, na.rm = TRUE),
                                                kurtosis_TransactionAmt_wday_card1 = kurtosis(TransactionAmt, na.rm = TRUE),
                                                sd_TransactionAmt_wday_card1 = sd(TransactionAmt, na.rm = TRUE)) %>%
  ungroup() %>%
  mutate(TransactionAmt_to_mean_wday_card1 = TransactionAmt/mean_TransactionAmt_wday_card1,
         TransactionAmt_to_sd_wday_card1 = TransactionAmt/sd_TransactionAmt_wday_card1,
         TransactionAmt_to_skewness_wday_card1 = TransactionAmt/skewness_TransactionAmt_wday_card1,
         TransactionAmt_to_kurtosis_wday_card1 = TransactionAmt/kurtosis_TransactionAmt_wday_card1,
         TransactionAmt_subs_wday_card1 = TransactionAmt - mean_TransactionAmt_wday_card1,
         TransactionAmt_subs_wday_card1_median = TransactionAmt - median_TransactionAmt_wday_card1)
...
tem <- tem %>% group_by(D0, card1) %>% mutate(mean_TransactionAmt_D0_card1 = mean(TransactionAmt, na.rm = TRUE),
                                              median_TransactionAmt_D0_card1 = median(TransactionAmt, na.rm = TRUE),
                                              skewness_TransactionAmt_D0_card1 = skewness(TransactionAmt, na.rm = TRUE),
                                              kurtosis_TransactionAmt_D0_card1 = kurtosis(TransactionAmt, na.rm = TRUE),
                                              sd_TransactionAmt_D0_card1 = sd(TransactionAmt, na.rm = TRUE)) %>%
  ungroup() %>%
  mutate(TransactionAmt_to_mean_D0_card1 = TransactionAmt/mean_TransactionAmt_D0_card1,
         TransactionAmt_to_median_D0_card1 = TransactionAmt/median_TransactionAmt_D0_card1,
         TransactionAmt_to_sd_D0_card1 = TransactionAmt/sd_TransactionAmt_D0_card1,
         TransactionAmt_to_skewness_D0_card1 = TransactionAmt/skewness_TransactionAmt_D0_card1,
         TransactionAmt_to_kurtosis_D0_card1 = TransactionAmt/kurtosis_TransactionAmt_D0_card1,
         TransactionAmt_subs_D0_card1 = TransactionAmt - mean_TransactionAmt_D0_card1,
         TransactionAmt_subs_D0_card1_median = TransactionAmt - median_TransactionAmt_D0_card1)
...
x <- data.matrix(x[ , 1:183])

tem_pca <- prcomp_irlba(x, scale. = TRUE)
...
temp <- data.frame(tem_pca, tem_ae)

#set.seed(71)
#iso <- isolationForest$new()
#iso$fit(impute(x, method = "median/mode"))
#temp$iforest_score <- iso$scores$anomaly_score
#temp$iforest_depth <- iso$scores$average_depth

tem <- data.frame(tem, temp)


rm(tem_pca, temp, x, sample.df.cor); gc(); gc()
...
write.csv(sub,"sub_0.9356_80.csv",row.names = F)
